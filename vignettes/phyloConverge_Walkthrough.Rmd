...
To start the analysis, we need to provide _phyloConverge_ with input data. The **neutral tree model** and the **alignment** can be loaded using the `read.tm` and `read.msa` functions in the _rphast_ package, respectively. The `read.msa` function also accepts alignments in the MAF format. Species names in the tree must match species names in the alignment. For the purposes of this vignette, we will use an alignment dataset previously published by [Roscito et al. (2018)](https://www.nature.com/articles/s41467-018-07122-z), with _mm10_ as the reference species. The conserved non-coding element (CNE) _CNE031689_ is used for illustration.
